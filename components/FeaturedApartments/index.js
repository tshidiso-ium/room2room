  ...
  useEffect(() => {
    const fetchApartments = async () => {
      try {
        ...
        setFilteredApartments(listings);
        setLoading(false);
      } catch (error) {
  ...
  useEffect(() => {
    const result = apartments.filter(apartment => {
      const matchesLocation =
        !filters?.location ||
        apartment.location?.city === filters.location ||
        apartment.location?.suburb === filters.location;
      ...
      const matchesBedrooms =
        !filters?.bedrooms ||
        apartment.bedrooms === parseInt(filters.bedrooms);
    ...
    <section className="py-10 px-4 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {filteredApartments.map((apt) => (
          console.log("Rendering apartment: ", apt.images),
          <Link
            key={apt.id}
